    ...
    <main class="full-width bg-black flex justify-center items-center py-3 transition">
        <div class="relative flex justify-center items-center transition ">
            ...
            <section id="login-wrapper" class="flex flex-column items-center transition">
                <a href="<?php echo e($data->site_url); ?>" class="login-head text-center w-full transition">
                    <img class="select-none w-full max-w-100px mx-auto" src="<?php echo e(asset($data->logo)); ?>" alt="site-logo" title="Unifying Caribbean People Socially Mentally $ Economically"
                         draggable="false"/>
                    <h1 class="text-white text-center text-3xl font-bold transition">Login</h1>
                </a>
                <form action="<?php echo e(route('admin.authenticate')); ?>" method="post" class="w-full transition"
                      id="login-form">
                    <?php echo csrf_field(); ?>
...
<?php $component = App\View\Components\Input::resolve(['label' => 'Email / Username','type' => 'text','name' => 'login','id' => 'login','placeholder' => 'Enter Email or Username'] + (isset($attributes) && $attributes instanceof Illuminate\View\ComponentAttributeBag ? $attributes->all() : [])); ?>
...
<?php endif; ?>
<?php $component->withAttributes(['iserror' => \Illuminate\View\Compilers\BladeCompiler::sanitizeComponentAttribute($errors->has('login')),'message' => \Illuminate\View\Compilers\BladeCompiler::sanitizeComponentAttribute($errors->first('login'))]); ?>
...
<?php endif; ?>
                    ...
                    <?php if(Route::currentRouteName() !== 'superadmin.login'): ?>
                        <div class="mb-08rem w-full transition text-end">
                            <a href="<?php echo e(route('admin.forget.password')); ?>" class="forget-password-link">forget password?</a>
                        </div>
                    <?php endif; ?>
                    <div class="mb-08rem w-full transition">
                        <button id="login-form-btn" class="form-login-btn text-black transition"
                                type="submit">Login
                        </button>
                    </div>
                    <?php if(Route::currentRouteName() !== 'superadmin.login'): ?>
                        <div class="w-full transition text-center mx-auto">
                            <p>Don't have an account? <a href="<?php echo e(route('admin.signup.index')); ?>"
                                                         class="forget-password-link">sign-up?</a></p>
                        </div>
                    <?php endif; ?>
                </form>
            </section>
    ...
    <script>
        $(document).ready(function () {
            $('#login-form').on('submit', function (e) {
                e.preventDefault();
                ajaxLoader('#login-form-btn', 'Login');
                const form = this;
                $('.text-primary').text('');
                const formData = $(this).serialize();
                $.ajax({
                    type: "POST",
                    url: "<?php echo e(route('admin.authenticate')); ?>",
                    data: formData,
...
<?php echo $__env->make('layout.app', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH E:\laragon\www\caribbean-airforce\resources\views/auth/login.blade.php ENDPATH**/ ?>