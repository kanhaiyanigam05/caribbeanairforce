...
class CustomCheckoutCalendar {
    ...
    init() {
        if (this.calendarType === "single-date" || this.calendarType === "multi-date") {
            // this.generateBaseCalendar();
            this.generateMultiDateBaseCalendar();
        } else {
            this.generateBaseCalendar();
        }
    }
    ...
    generateMultiDateBaseCalendar() {
        const parentWrapper =
            this.calendarButtonInput.parentElement.parentElement;

        this.calendar = parentWrapper.querySelector(".container-new");
        this.calendar ? this.calendar.remove() : (this.calendar = null);
        const calendarHtml = `
                <div class="container-new">
                    <div class="custom-calendar">
        ...
        `;
        let calenderWrapper = parentWrapper.querySelector(
            ".custom-ticket-calender"
        );
        if (this.calendarType === "multi-date") {
            calenderWrapper = parentWrapper.querySelector(
                ".custom-ticket-calender1"
            );
        }
        calenderWrapper.innerHTML = calendarHtml;
        // this.calendarButtonInput.insertAdjacentHTML("afterend", calendarHtml);
        ...
        this.errorElement = this.errorWrapper.querySelector(".error-message");
        this.calendarSaveButton =
            parentWrapper.querySelector(".save-calendar-btn");
        this.calendarCancelButton = parentWrapper.querySelector(
            ".cancel-calendar-btn"
        );
        this.attachEventListeners();
    }
        ...
        const buildCalendarView = (calendarData) => {
            ...
            for (let i = 1; i <= numberOfDays; i++) {
                ...
                if (calendarData.disablePastDates) {
                    ...
                    const defaultDateISOString = `${nowUTC.toISOString().split("T")[0]
                        }T00:00:00.000Z`;
    ...
    singleDaySelection(options) {
        ...
        if (this.calendarType === "single-date") {
            ...
            if (Object.keys(this.singleDate.selectedDate).length >= 1) {
                const dateString = `${this.singleDate.selectedDate.date.split("T")[0]
                    }T00:00:00.000Z`;
            ...
            allDateElements.forEach((day) => {
                day.addEventListener("click", () => {
                    ...
                    if (!this.singleDate.selectedDate.time) {
                        this.singleDate.selectedDate.time = [{}]; // Initialize with an empty object at index 0
                    }
                    // this.singleDate.selectedDate.time[0].startTime =
                    //     this.singleDate.dataSet.time[0].startTime;
                    // this.singleDate.selectedDate.time[0].endTime =
                    //     this.singleDate.dataSet.time[0].endTime;
        ...
        data.selectedDates.forEach((selected) => {
            ...
            if (datesBetween.length > 0) {
                ...
                const activeEndDate = this.calendarDaysWrapper.querySelector(
                    `div[data-date="${datesBetween[datesBetween.length - 1]
                    }T00:00:00.000Z"]`
                );
    ...
    generateDateDisplay() {
        ...
        if (this.calendarType === "single-date") {
            ...
            document.querySelector(".pass-date-detail2").textContent = `Date: ${date}`;
            /*if (wrapper) {
        ...
        if (this.calendarType === "multi-date") {
            ...
            document.querySelector(".pass-date-detail1").textContent = `Selected Slots: ${validLength}`;
            ...
            if (this.multiDate.selectedDates.length === 0) {
                ...
                if (validSelectedDates === 0) {
                    ...
                    if (validSelectedDates === 1) {
                        ...
                        const buttonText = `${firstDate} | ${firstStartTime} - ${firstEndTime} and ${validSelectedDates - 1
                            } more...`;
            ...
            if (this.multiRangeDate.selectedDates.length === 0) {
                ...
                if (selectedSlotsLength >= 1) {
                    if (selectedSlotsLength === 1) {
                        ...
                        const buttonsText = `${firstStartDate} to ${firstEndDate} | ${firstStartTime} to ${firstEndTime} and ${selectedSlotsLength - 1
                            } more...`;
                        this.calendarButtonInput.value = buttonsText;
                        this.calendarButtonInputText.textContent = buttonsText;
                    }
...
function handleSeperateType(type, event) {
    ...
    let calenderarea = typeareapass.querySelector(".custom-calendar-wrapper")
    let calenderarea1 = typeareaticket.querySelector(".custom-calendar-wrapper")
    ...
    if (type === "pass") {
        ...
        singleDayCalendar.singleDaySelection({
            defaultDate: event.slots[0].start_date,
        ...
        document.querySelector(".pass-time-detail2").innerHTML = `Time: ${event.slots[0].time[0].start_time} -  ${event.slots[0].time[0].end_time}`;
    }
    else {
        typeareapass.classList.add("hidden");
        typeareaticket.classList.remove("hidden");
        calenderarea1.classList.add("multi-date")
        ...
        multiDayCalendar.multiDaySelection({
            defaultDate: event.slots[0].date,
            disablePastDates: true,
            defaultButtonText: "Ticket",
            dataSet: ticketvalue
        });
...
function modifyForCustomDate(data) {
    ...
    data.ticket_slots = formattedSlots.slots;
...
function handlePassSaveButton() {
    ...
    let inputarea = document.querySelector('.hidden-slots-wrapper')
    if (inputarea !== null) {
    ...
    parentDiv.appendChild(slotDatesInput);
    document.querySelector(".container-new").appendChild(parentDiv);
...
function handleStatickPassSelected(index) {
    ...
    savedata.startDate = savedata.start_date;
    savedata.endDate = savedata.end_date;
    let daycount = countDays(savedata.startDate, savedata.endDate)
    savedata.packages = calculatePackagePrice(savedata.packages, daycount)
    let newdata = {
        selectedDates: [savedata],
        dataSet: ticketvalue,
    };
     ...
     <div class="calender-area mt-2">
                                                  <div class="">
                                                      <div class="calendar-container-new">
                                                          <div class="calendar-inner-container-new flex">
                                                              <div class="container-new m-auto">
...
function handleDateSelection(dayDiv, dayString) {
    ...
    let passcalendererror = document.querySelector('.pass-calender-pick-error')
    customsavedata = null
    ...
    if (avilable_day >= selectcustomdays) {
        ...
        // }
        passcalendererror.classList.add('hidden')
    }
    else {
        passcalendererror.classList.remove('hidden')
        passcalendererror.innerText = `Avilable days are less then ${selectcustomdays}`
    }
...
function getSelectedSlotForDate() {
    ...
    let modifieddata = {
        ...
        "packages": calculatePackagePrice(data.packages, applySelectiondata.length),
...
function calculatePackagePrice(packages, multiplier) {
    return packages.map(pkg => ({
        ...pkg,
        price: pkg.price * multiplier
    }));
}
...
function handleChangeTab(element) {
    if (element === "pass") {
        ...
        document.querySelector('.custom-tab-button-ticket1').classList.remove('bg-white')
        document.querySelector('.custom-tab-button-ticket1').classList.add('text-white')
    } else {
        ...
        document.querySelector('.custom-tab-button-ticket1').classList.add('bg-white')
        document.querySelector('.custom-tab-button-ticket1').classList.remove('text-white')
    }
