var dummyData = [
    {
        "id": 1,
        "title": "Accessibility",
        "icon": "https://www.svgrepo.com/download/521938/accessibility-symbol.svg"
    },
    {
        "id": 2,
...
function handleClickSubmitPrice(element) {
    ...
    if (value !== '') {
        ...
        amenitesData.push({id:id,name: name, price: value })
    }
    else {
...
function handleClickRemovePrice(element) {
    ...
    let name = card.querySelector('.amenities-heading').innerText
    amenitesData = amenitesData.filter(item => item.id !== card.id);
...
function createSelectedAmenties() {
    ...
    amenitesData.forEach(item => {
        ...
        let card = `
                                               <div class="amenities-card flex gap-6 red mb-1" id=${item.id}  >
                                                    <div class="flex gap-3" style="align-items: center;">
                                                        <div>
                                                            <svg xmlns="http://www.w3.org/2000/svg" width="40" height="41" viewBox="0 0 40 41" fill="none">
                                                                <g clip-path="url(#clip0_59_27)">
...
function handleRemoveSelectedAmenties(element) {
    ...
    handleRemoveCardFromModal(card.id)
}


function createAmentiesCard() {

    let a = document.querySelector('.amenities-card-area')
    dummyData.forEach(item => {
        let card = `
                                  <div class="amenities-card flex gap-6" id=${item.id}>
                                                <div class="flex gap-3" style="align-items: center;" >
                                                    <div>
                                                        <img src=${item.icon} alt='icon' width="40" height="41"  />
                                                    </div>
                                                    <div>
                                                        <p class="amenities-heading">${item.title}</p>
...
function handleRemoveCardFromModal(id){
    const cardarea = document.querySelector('.amenities-card-area')
    const card = document.getElementById(id);   // <- change here
