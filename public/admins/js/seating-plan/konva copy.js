class KonvaSeatingPlan {
    ...
    _initProperties(container, width, height) {
        ...
        console.log(this.layoutData.type);
    ...
    _initStage() {
        if (this.stage) {
            this.stage.destroy(); // Completely removes DOM and listeners
            this.stage = null;
        }
    ...
    _findTierOfSeat(seat) {
        return Object.values(this.tiers).find(t => t.seats.includes(seat.id()));
    }
    _bindZoomControls() {
    ...
    drawGrid() {
        ...
        const oldGrid = this.layer.findOne('.grid-lines');
    ...
    addAmenities() {
        ...
        const visibleAmenities = this.layoutData.amenities || [];
    ...
    handleTierSeatClick(seat) {
        ...
        const tier = this.tiers[selectedTierId];

        for (const [tierId, t] of Object.entries(this.tiers)) {
    ...
    updateHoldOverlays() {
        Object.entries(this.holds).forEach(([holdId, hold]) => {
        ...
        overlay.addEventListener("click", () => {
            ...
            selectedSeats.forEach(seat => {
                ...
                if (isRemoving) {
                    ...
                    Object.values(this.holds).forEach(h => {
    ...
    updateSeatColors() {
        ...
        seats.forEach(seat => {
            ...
            const tier = this.tiers[metadata?.tierId];
            const hold = this.holds[metadata?.holdId];
    ...
    setSeatColor({ seat, fillColor = null, strokeColor = null, selected = false, reset = false }) {
        ...
        const inTier = tierId && this.tiers[tierId];
        const holdId = metadata?.holdId;
        const inHold = holdId && this.holds[holdId];
        ...
        if (this.activeTab === 'tier' || this.activeTab === 'hold') {
            if (selected) {
                if (this.activeTab === "hold" && inTier && inHold) {
                    ...
                    seat.stroke(this.tiers[tierId].color);
    ...
    setActiveTab() {
        const tabsWrapper = this.mainWrapper.querySelector(".map-head .left");
    ...
    isSeatInAnyTier(seat) {
        const seatId = seat.id();
        return Object.values(this.tiers).some(tier => tier.seats.includes(seatId));
    }
    ...
    exportSeatingPLan() {
        console.log("Export seating plan");
