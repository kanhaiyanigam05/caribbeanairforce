    ...
    <main class="full-width bg-black flex justify-center items-center py-3 transition">
        <div class="relative flex justify-center items-center transition ">
            ...
            <section id="login-wrapper" class="flex flex-column items-center transition">
                <a href="{{ $data->site_url }}" class="login-head text-center w-full transition">
                    <img class="select-none w-full max-w-100px mx-auto" src="{{ asset($data->logo) }}" alt="site-logo" title="Unifying Caribbean People Socially Mentally $ Economically"
                         draggable="false"/>
                    <h1 class="text-white text-center text-3xl font-bold transition">Login</h1>
                </a>
                <form action="{{ route('admin.authenticate') }}" method="post" class="w-full transition"
                      id="login-form">
                    @csrf
                    <x-input label="Email / Username" type="text" name="login" id="login"
                             placeholder="Enter Email or Username" :iserror="$errors->has('login')"
                    ...
                    @if(Route::currentRouteName() !== 'superadmin.login')
                        <div class="mb-08rem w-full transition text-end">
                            <a href="{{ route('admin.forget.password') }}" class="forget-password-link">forget password?</a>
                        </div>
                    @endif
                    <div class="mb-08rem w-full transition">
                    ...
                    @if(Route::currentRouteName() !== 'superadmin.login')
                        <div class="w-full transition text-center mx-auto">
                            <p>Don't have an account? <a href="{{ route('admin.signup.index') }}"
