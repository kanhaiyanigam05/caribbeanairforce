        ...
        <section id="login-wrapper" class="flex flex-column items-center transition">
            <a href="{{ route('admin.dashboard') }}" class="login-head text-center w-full transition">
                <img class="select-none w-full max-w-100px mx-auto"
                    ...
                    draggable="false" />
                <h1 class="text-white text-center text-3xl font-bold transition">Forget Password?</h1>
            </a>
            <form action="{{ route('admin.send.reset.link') }}" method="post" class="w-full transition"
                id="forget-password-form">
                @csrf
                <x-input label="Email" type="email" name="email" id="email" placeholder="Email"
                    :iserror="$errors->has('email')" :message="$errors->first('email')" />

                <div class="mb-08rem w-full transition">
                    <button id="forget-password-btn" class="form-login-btn text-black transition" type="submit">Reset My
                        Password</button>
                </div>
...
<script>
    $(document).ready(function() {
            $('#forget-password-form').on('submit', function(e) {
                e.preventDefault();
                ajaxLoader('#forget-password-btn', 'Login');
                const form = this;
                $('.text-primary').text('');
                const formData = $(this).serialize();
                $.ajax({
                    type: "POST",
                    url: "{{ route('admin.send.reset.link') }}",
                    data: formData,
