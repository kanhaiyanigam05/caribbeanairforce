    ...
    <main>
        ...
        <section class="relative full-width max-h-400px">
            <div class="h-250px flex justify-end items-center">
                <img class="w-full select-none object-cover h-250px" id="cover-image-display"
                    src="{{ Auth::user()->banner }}" alt="banner" draggable="false" />
                ...
                <div class="relative rounded-full user-profile-img-wrapper">
                    ...
                    <div class="relative rounded-full profile-display-image">
                        <img class="absolute select-none rounded-full w-full h-full object-cover" id="profile-image-display"
                            src="{{ Auth::user()->profile }}" alt="" draggable="false" />
                    </div>
                </div>
                    ...
                    <div class="w-full flex justify-between items-center details-wrapper-main">
                        <div class="details-wrapper">
                            <div class="flex justify-start items-center gap-1">
                                ...
                                <h5 class="user-name-2">{{ Auth::user()->username }}</h5>
                            </div>
                            <p class="designation">
                                @if (Auth::user()->role === \App\Enums\Role::SUPERADMIN)
                                    Super Admin
                                @elseif (Auth::user()->role === \App\Enums\Role::ORGANIZER)
                                    Event Organizer
                                @else
                                    User
                                @endif
                            ...
                            <div class="mb-03rem flex justify-start items-center gap-05 profile-connections-wrapper">
                                <p class="connections">{{ Auth::user()->followers->count() }} <span>follower
                                        {{ Auth::user()->followers->count() > 1 ? 's' : '' }}</span>
                                </p>
                                <div class="flex justify-start items-center">
                                    @foreach (Auth::user()?->followers?->take(5) as $follower)
                                        <img class="select-none object-cover rounded-full thub-connections-img"
                                            src="{{ $follower->profile }}" alt="{{ $follower->full_name }}" draggable="false" />
                                    @endforeach
                                </div>
                            </div>
                        </div>
        ...
        <section class="events-section-inner profile-tab-menu-main">
            <div class="bottom-edge-shadow flex items-center gap-3rem nav-2-post-event">
                    ...
                    <div class="profile-tab-menu-wrapper" style="flex: 0 0 0">
                        <div class="relative" onclick="handleDropdown(this)">
                            ...
                            <div class="absolute dropdown-wrapper">
                                <div class="dropdown">
                                    <a href="{{ route('admin.events.upcoming') }}">Upcoming Events</a>
                                    <a href="{{ route('admin.events.past') }}">Past Events</a>
                        ...
                        <div class="relative" onclick="handleDropdown(this)">
                            ...
                            <div class="absolute dropdown-wrapper">
                                <div class="dropdown">
                                    <a href="{{ route('admin.suggestions') }}">Suggestions</a>
                                    <a href="{{ route('admin.following') }}">Following</a>
                                    <a href="{{ route('admin.followers') }}">All Followers</a>
                                </div>
                            </div>
                        </div>
                ...
                @endif
                @if (Auth::user()->role !== \App\Enums\Role::USER || Auth::user()->block == true)
    ...
    <script>
        ...
        $(document).ready(function() {
            $('#personal-info-form').on('submit', function(e) {
                ...
                $.ajax({
                    type: "POST",
                    url: "{{ route('admin.signup.store.setting') }}",
                    data: formData,
                    processData: false,
                    contentType: false,
                    success: function(data) {
        ...
        $(document).ready(function() {
            $('#password-reset-form').on('submit', function(e) {
                ...
                $.ajax({
                    type: "POST",
                    url: "{{ route('admin.signup.store.password') }}",
                    data: formData,
                    processData: false,
                    contentType: false,
                    success: function(data) {
