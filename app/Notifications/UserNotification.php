...
class UserNotification extends Notification
{
    ...
    public function toMail($notifiable)
    {
        ...
        if(property_exists($this->object, 'role') && $this->object->role){
            return (new MailMessage)
                ->subject($this->message)
                ->line($this->message)
                ->line('User Details are below:')
                ->line('Email: ' . $this->object?->user?->email)
                ->line('Username: ' . $this->object?->user?->username)
                ->line('Password: *********')
